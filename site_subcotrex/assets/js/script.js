...
$(document).ready(function($) {
	...
	ibg();	
    function ibgVideo() {
        $.each($('.ibg-video'), function(index, val){
            let video = $(this).find('video');
            if(video.length > 0) {
                let videoSrc = video.find('source').attr('src'); // Pobieramy ścieżkę do wideo
                $(this).css({
                    'background': 'url("' + videoSrc + '") no-repeat center center',
                    'background-size': 'cover'
                });
                video.hide(); // Ukrywamy oryginalne wideo, jeśli nie jest potrzebne
            }
        });
    }		
